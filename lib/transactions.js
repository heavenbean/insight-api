...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  transformed.valueOut = transaction.outputSatoshis / dashcore.valuetoSatoshi;
  transformed.size = transaction.hex.length / 2; // in bytes
  if (!transaction.coinbase) {
    transformed.valueIn = transaction.inputSatoshis / dashcore.valuetoSatoshi;
    transformed.fees = transaction.feeSatoshis / dashcore.valuetoSatoshi;
  }
...
TxController.prototype.transformInput = function(options, input, index) {
  ...
  transformed.value = input.satoshis / dashcore.valuetoSatoshi;
...
TxController.prototype.transformOutput = function(options, output, index) {
  var transformed = {
    value: (output.satoshis / dashcore.valuetoSatoshi).toFixed(8),
    n: index,
    scriptPubKey: {
      hex: output.script
    }
...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  var transformed = {
    txid: transaction.hash,
    valueOut: valueOut / dashcore.valuetoSatoshi,
    vout: vout,
    isRBF: isRBF,
    txlock: false
  };
